...
class cl_toolbar :
  def __init__( self, master ) :
    ...
    dummy = tk.Button( self.toolbar, text = 'φ', width = 2, command = self.phi_callback ) 
    dummy.pack( side = tk.LEFT, padx = 2, pady = 2 )
  ...
  def resolution_callback( self ) :
    f = simpledialog.askinteger( "Title", "Prompt?",
              initialvalue = self.master.resolution, minvalue = 4 )
    if f is None:
      self.master.statusBar_frame.set( 'Resolution was cancelled' )
    else:
      self.master.resolution = f
      self.master.statusBar_frame.set( f'Resolution: {self.master.resolution}, Distance: {self.master.distance}, φ: {round(math.degrees(self.master.phi), 1)}°  θ: {round(math.degrees(self.master.theta), 1)}°  ψ: {round(math.degrees(self.master.psi), 1)}°' )
  
  def distance_callback( self ) :
    f = simpledialog.askfloat( "Title", "Prompt?",
              initialvalue = self.master.distance, minvalue = 0 )
    if f is None:
